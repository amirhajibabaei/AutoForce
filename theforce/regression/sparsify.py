...
def sparsify_saved_model(
    input_model, alpha=1.0, sweeps=10, output_model=None, report=True, plot=True
):

    if output_model is None:
        output_model = (
            os.path.join(os.path.dirname(input_model), os.path.basename(input_model))
            + "_sparse"
        )
        j = 0
        suffix = ""
    ...
    V = PosteriorPotentialFromFolder(input_model, load_data=True, update_data=False)
    M = V.M
    K = V.K
    ...
    indices = None
    deleted = None
    K, M, indices, deleted = sparser_projection(
        K, M, Y, D, alpha=alpha, sweeps=sweeps, indices=indices, deleted=deleted
    )
    V_M = V.M
    V_K = V.K
    V_mu = V.mu
    ...
    if report:
        ...
        with open(os.path.join(output_model, "_sparse"), "w") as report:
            ...
            report.write(
                "{} + {} = {} ? {}\n".format(
                    len(deleted), len(i), len(deleted) + len(i), V_M.size(0)
                )
            )
    ...
    if plot:
        ...
        _ = plt.hist(
            (V_K @ V_mu - Y).detach(), bins=100, color="cyan", alpha=0.5, label="full"
        )
        _ = plt.hist(
            (K @ mu - Y).detach(), bins=100, color="green", alpha=0.5, label="sparse"
        )
        plt.legend()
