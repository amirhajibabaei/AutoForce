...
class Radii:
    ...
    def __call__(self, numbers):
        return torch.cat(
            [torch.as_tensor(self.get(int(num))).view(1) for num in numbers]
            + [torch.empty(0)]
        )
...
class SpecialRadii(Radii):
    def __init__(self, dct, others=1.0):
        self.dct = dct
        self.others = others
...
class DefaultRadii(Radii):
    def __init__(self, default=1.0, special={1: 0.5}):
...
class SeSoap(Module):
    def __init__(self, lmax, nmax, radial, radii=1.0, flatten=True, normalize=True):
        super().__init__()
        self.ylm = Ylm(lmax)
        self.nmax = nmax
        self.radial = radial
...
class SubSeSoap(Module):
    def __init__(
        self, lmax, nmax, radial, numbers, radii=1.0, flatten=True, normalize=True
    ):
        super().__init__()
        self.ylm = Ylm(lmax)
        self.nmax = nmax
...
def test_SeSoap():
    ...
    radii = {10: 0.8, 11: 1.0, 18: 1.2, 19: 1.4}
