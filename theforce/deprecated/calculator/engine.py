...
def _input(systems):
    energies = torch.tensor([atoms.get_potential_energy() for atoms in systems])
    forces = torch.cat([torch.tensor(atoms.get_forces()) for atoms in systems]).view(-1)
...
def example():
    ...
    print(
        "isclose: {} (diff = {})".format(
            np.isclose(dmin, 2 ** (1.0 / 3)), dmin - 2 ** (1.0 / 3)
        )
    )
