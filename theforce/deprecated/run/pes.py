...
def get_params(**kwargs):
    ...
    for a, b in kwargs.items():
        if a not in params:
            raise RuntimeWarning("kwargs: keyword {} is not recognized".format(a))
            continue
        params[a] = b
...
def get_kernel(params):
    ...
    from theforce.regression.kernel import (
        Add,
        DotProd,
        Mul,
        Normed,
    ...
    from theforce.similarity.soap import NormedSoapKernel, SoapKernel

    # Gaussian Process
    if (
        params["path_gp_chp"]
        and params["use_gp_chp"]
        and os.path.isfile(params["path_gp_chp"])
    ):
        ...
        if params["pairkernel"]:
            pairs = [
                (a, b) for a, b in itertools.combinations(params["numbers"], 2)
        ...
        if params["soapkernel"]:
            ...
            kerns += [
                SOAP(
                    Positive(1.0, requires_grad=True) * DotProd() ** params["exponent"],
                    atomic_number,
                    params["numbers"],
                    params["lmax"],
                    params["nmax"],
        ...
        gp = GaussianProcessPotential(
            kerns,
            noise=White(signal=params["noise"], requires_grad=params["noisegrad"]),
        )
...
def potential_energy_surface(
    data=None, inducing=None, train=0, caching=False, append_log=True, **kwargs
):
    from theforce.descriptor.atoms import AtomsData, LocalsData, sample_atoms
    from theforce.regression.gppotential import PosteriorPotential, train_gpp
    ...
    if data is None:
        data = sample_atoms(
            params["path_data"], size=params["ndata"], chp=params["path_data_chp"]
        )
    ...
    if params["path_inducing_chp"] is not None:
        ...
        log.write("path_inducing_chp: {}\n".format(params["path_inducing_chp"]))
    ...
    for steps in iterable(train):
        train_gpp(
            gp, data, inducing=inducing, steps=steps, logprob_loss=True, cov_loss=False
        )
        # save gp
        state += steps
        ...
        if inducing.trainable:
            raise NotImplementedError("trainable inducing is not implemented yet!")
    if state > 0:
        with open("log.txt", "a") as log:
            log.write("\ntrained for {} steps\n".format(state))
