...
from theforce.descriptor.atoms import AtomsData, LocalsData, TorchAtoms, sample_atoms
        ...
        if (tol > tolerance).any():
            ...
            if ndft % retrain_every == 0:
                ...
                for steps in iterable(retrain):
                    atoms.calc.potential.train(
                        data, inducing=inducing, steps=steps, cov_loss=False
                    )
            ...
            print(
                "|............... : old max-error: predicted={}, true={}".format(
                    _err_pred, _err
                )
            )
            print(
                "|............... : new max-error: predicted={}, true={}".format(
                    err_pred, err
                )
            )
            arrays = np.concatenate([true_forces, _forces, forces, _var, var], axis=1)
...
def example():
    ...
    vel = np.random.uniform(-1.0, 1.0, size=ini_atoms.positions.shape) * 1.0
    vel -= vel.mean(axis=0)
