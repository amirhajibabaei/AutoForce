...
from ase.units import GPa, Pascal
...
from theforce.run.fly import default_kernel, fly, numpy_same_random_seed, suffixed_new
from theforce.similarity.universal import UniversalSoapKernel
from theforce.util.aseutil import get_repeat_reciprocal
from theforce.util.ssh import clear_port, forward_port
...
def aneal(
    ...
    if atoms is not None:
        ...
        # size
        if siz is not None:
            repeat = get_repeat_reciprocal(atoms, 1.0 / siz)
    ...
    if atoms is None:
        ...
        if len(numbers < 4):
            kern = default_kernel(
                numbers, cutoff=rc, lmax=lmax, nmax=nmax, exponent=eta
            )
        else:
    ...
    # params
    kw = dict(dt=dt, calc=calc, ediff=ediff, group=dist.group.WORLD, kern=kern)
            ...
            try:
                if s == suff:
                    fly(
                        ...
                        jumps,
                        atoms=atoms,
                        lf_kwargs=dict(restrict=restrict, volatile=volatile),
                        **kw,
                    )
                else:
...
if __name__ == "__main__":
    ...
    # arguments
    parser.add_argument(
        "-a", "--atoms", default=None, help="path to a file to read atoms"
    )
    parser.add_argument(
        "-r", "--repeat", default="1,1,1", help="repeat to make a supercell"
    )
    parser.add_argument(
        "-t", "--temperatures", default="300,600,1000", help="e.g. 300,600,1000"
    )
    parser.add_argument("-p", "--pressure", type=float, default=None, help="GPa")
    parser.add_argument("-m", "--modulus", type=float, default=None, help="GPa")
    parser.add_argument(
        "-s", "--size", default="10.", help="Angstrom, also could be None"
    )
    parser.add_argument("-j", "--jumps", type=int, default=100, help="training trials")
    parser.add_argument(
        "-std",
        "--standard",
        type=int,
    ...
    parser.add_argument(
        "-res", "--restrict", default=None, help="atomic numbers e.g. 3,15,16"
    )
    parser.add_argument(
        "-v",
        "--volatile",
    ...
    if args.atoms is None:
        ...
        atoms = (
            read(args.atoms, -1) if args.atoms.endswith(".traj") else read(args.atoms)
        ).repeat(repeat)
    ...
    #
    restrict = (
        None if args.restrict is None else [int(z) for z in args.restrict.split(",")]
    )
