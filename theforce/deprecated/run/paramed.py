...
from theforce.calculator.parametric import (
    get_coulomb_terms,
    get_lj_terms,
    load_parametric_potential,
)
from theforce.regression.scores import coeff_of_determination


def train_parametric_potential(
    data,
    cutoff=None,
    paramed_pot=None,
    chp="paramedpot.chp",
    ...
    # read
    if paramed_pot is None and os.path.isfile(chp) and use_chp:
    ...
    if paramed_pot is None:
        ...
        paramed_pot = get_coulomb_terms(
            numbers, cutoff, setting=charges_setting
    ...
    for _ in range(max_steps):
        # step
        optimizer.zero_grad()
        pred = torch.cat([paramed_pot(atoms, forces=True)[1] for atoms in data])
        ...
        paramed_pot.to_file(
            chp,
            flag="step: {}   r2 on all forces: {}   r2 on large forces = {}".format(
