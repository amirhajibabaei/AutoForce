...
def my_vasp(**kwargs):
    ...
    calc = Vasp(
        command="mpirun -np $NSLOTS vasp_std",
        setups="recommended",
        lreal="Auto",
        istart=0,
        **kwargs,
...
def default_kernel(
    numbers, cutoff=6.0, au=None, exponent=4, lmax=3, nmax=3, noise=0.01
):
    from theforce.descriptor.cutoff import PolyCut
    from theforce.regression.gppotential import GaussianProcessPotential
    from theforce.regression.kernel import DotProd, Positive, White
    ...
    kerns = [
        SOAP(
            ...
            nmax,
            PolyCut(cutoff),
            atomic_unit=au(a) if au else default_au(a),
        )
        for a in numbers
    ]
    gp = GaussianProcessPotential(kerns, noise=White(signal=noise, requires_grad=False))
