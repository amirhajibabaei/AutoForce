...
class ConstMean(Module):
    def __init__(self, c=0.0, requires_grad=True):
        super().__init__()
...
class Covariance(Module):
    ...
    def __init__(self, kernels):
        super().__init__()
        self.kernels = kernels if hasattr(kernels, "__iter__") else (kernels,)
        self.params = [par for kern in self.kernels for par in kern.params]

    def base_kerns(self, x=None, xx=None, operation="func"):
        return torch.stack(
            [kern(x=x, xx=xx, operation=operation) for kern in self.kernels]
        ).sum(dim=0)
    ...
    def diag(self, x=None, operation="func"):
        ...
        return sign * torch.stack(
            [kern.diag(x=x, operation=operation) for kern in self.kernels]
        ).sum(dim=0)
...
class Inducing(Covariance):
    def __init__(self, kernels, x, num=None, learn=False, signal=5e-2):
        ...
        self.xind = Parameter(
            x.clone() if num is None else x[torch.randint(0, x.size(0), (num,))],
            requires_grad=learn,
        )
    ...
    def decompose(self, x=None, xx=None, operation="func"):
        if x is None and xx is None:
            ...
            xx = x
        left = (
            self.leftgrad(x, self.xind)
            if operation == "leftgrad" or operation == "gradgrad"
            else self.base_kerns(x, self.xind)
        )
        right = (
            self.rightgrad(self.xind, xx)
            if operation == "rightgrad" or operation == "gradgrad"
            else self.base_kerns(self.xind, xx)
        )
    ...
    def cov_factor(self, x, operation="func"):
        ...
        cov_loss = (
            0.5
            * (self.diag(x, operation=operation).sum() - torch.einsum("ij,ij", Q, Q))
            / self.white.diag()
        )
    ...
    def forward(self, x, op="func"):
        ...
        if hasattr(self.cov, "cov_factor"):
            ...
            return MultivariateNormal(
                self.mean(x, operation=op).view(-1),
...
class PosteriorGP(Module):
    ...
    def forward(self, X):
        raise NotImplementedError(
            "".join(
                (
                    "Similar to GaussianProcess class, this should return",
...
def test_basic():
    ...
    from theforce.regression.core import LazyWhite, SquaredExp
