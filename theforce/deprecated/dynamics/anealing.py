    ...
    for k, target_t in enumerate(temperatures):
        ...
        while (heating and t < target_t) or (cooling and t > target_t):
            dyn.rescale_velocities(
                rescale_velocities if heating else 1.0 / rescale_velocities
            )
            _, e, t, s = dyn.run_updates(equilibration)
        if k == stages - 1:
            dyn.model.to_folder(
                name, info="temperature: {}".format(t), overwrite=overwrite
            )
        else:
            dyn.model.to_folder(
                "{}_{}".format(name, k),
...
def learn_pes_by_tempering(
    ...
    for k, target_t in enumerate(checkpoints):
        print(
            "stage: {}, time: {}, target time: {}, (temperature={})".format(
                k, t, target_t, T
            )
        )
        while t < target_t:
            ...
            if pressure is not None:
                ...
                # figure out strain
                dp = p - pressure
                strain = (rescale_cell if dp > 0 else 1.0 / rescale_cell) - 1
                if eps == "random":
    ...
    for k, target_t in enumerate(checkpoints):
        ...
        while t < target_t:
            ...
            if pressure is not None:
                ...
                # figure out factor
                dp = p - pressure
                factor = rescale_cell if dp > 0 else 1.0 / rescale_cell
                if randomize:
