...
class SeSoap:
    def __init__(self, lmax, nmax, radial, modify_scale=None):
        ...
        self._m = [
            (
                [l for m in range(0, l + 1)] + [m for m in range(0, l)],
        ...
        byte = _ar.strides[0]
        self._il = self.compress(
            np.lib.stride_tricks.as_strided(
                _ar, shape=(lmax + 1, nmax + 1, nmax + 1), strides=(byte, 0, 0)
            ),
            "lnn",
        )
        self._in = self.compress(
            np.lib.stride_tricks.as_strided(
                _ar, shape=(lmax + 1, nmax + 1, nmax + 1), strides=(0, byte, 0)
            ),
            "lnn",
        )
        self._inp = self.compress(
            np.lib.stride_tricks.as_strided(
                _ar, shape=(lmax + 1, nmax + 1, nmax + 1), strides=(0, 0, byte)
            ),
            "lnn",
        )
    ...
    def derivatives(
        self, xyz, order=0, normalize=False, sumj=True, cart=True, weights=None
    ):
        """
        Inputs:   xyz -> Cartesian coordinates, see parse_xyz
        order:    0 or 1, see parse_xyz
    ...
    def sph_vec_to_cart(sin_theta, cos_theta, sin_phi, cos_phi, F_r, F_theta, F_phi):
        F_x = (
            sin_theta * cos_phi * F_r + cos_theta * cos_phi * F_theta - sin_phi * F_phi
        )
        F_y = (
            sin_theta * sin_phi * F_r + cos_theta * sin_phi * F_theta + cos_phi * F_phi
        )
    ...
    def sum_all_m(self, c):
        ...
        if rank == 6:
            return np.array(
                [
                    2 * c[:, :, self._m[l][0], self._m[l][1], :, :].sum(axis=-3)
                    - c[:, :, l, l, :, :]
            ...
            return np.array(
                [
                    2 * c[:, :, self._m[l][0], self._m[l][1], :].sum(axis=-2)
                    - c[:, :, l, l, :]
                    for l in range(self.lmax + 1)
                ]
            )
            ...
            return np.array(
                [
                    2 * c[:, :, self._m[l][0], self._m[l][1]].sum(axis=-1)
                    - c[:, :, l, l]
                    for l in range(self.lmax + 1)
                ]
            )
    ...
    def compress(self, a, type):
        if type == "lnn":
            ...
            print(
                "type {} not defined yet, for matrix with shape {}".format(
                    type, a.shape
                )
            )
    ...
    def __approx_plnn__(self, l, n, m, r_c, rho):
        ...
        return (
            np.sqrt(2 * l + 1)
            * self.__approx_cnlm__(l, n, r_c, rho)
            * self.__approx_cnlm__(l, m, r_c, rho)
            * self.lnnp_c[0]
        )
    ...
    def __opt_rc__(self, rho):
        ...
        def objective(rc):
            p = self.__approx_plnn__(self._il, self._in, self._inp, rc, rho)
            norm = np.linalg.norm(p)
            p /= norm
            return p.var()
...
def test_sesoap():
    ...
    env = SeSoap(2, 2, quadratic_cutoff(3.0), modify_scale=-1.0)
    p_ = env.descriptor([x, y, z], order=1, normalize=False)
    p_dc, q_dc = env.derivatives([x, y, z], order=1, normalize=False, cart=False)
    ...
    ref_p = [
        ...
        np.array(
            [
                [
                    [-0.73777549, -0.05089412, 0.74691856],
    ...
    pj, qj = env.derivatives((x, y, z), order=1, sumj=False, cart=True, normalize=False)
    # order stuff
    ...
    pj, qj = env.derivatives((x, y, z), order=1, sumj=True, cart=True, normalize=False)
    qj = qj.T  # order stuff
    ...
    p, q = env.derivatives([x, y, z], order=1, sumj=False, cart=True, normalize=True)
    pp, qq = env.derivatives(
        (xx, yy, zz), order=1, sumj=False, cart=True, normalize=True
    )
    q, qq = (np.transpose(a, axes=(1, 0, 2)) for a in [q, qq])  # order stuff
...
def test_sesoap_performance(n=30, N=100):
    ...
    start = time.time()
    for _ in range(N):
        x, y, z = (np.random.uniform(-1.0, 1.0, size=n) for _ in range(3))
    finish = time.time()
    ...
    for _ in range(N):
        ...
        xyz = np.random.uniform(-1.0, 1.0, size=(n, 3))
...
def test_derivatives(n=20, rc=1.0, normalize=True, N=100, atol=1e-10):
    """
    ...
    for _ in range(N):
        ...
        p1, q = env.derivatives(
            [x, y, z], order=1, normalize=normalize, sumj=False, weights=weights
        )
        ...
        p2 = env.descriptor([x, y, z], order=1, normalize=normalize, weights=weights)
        fd = (p2 - p1) / delta
        ...
        y[j] += delta * u[1]
        z[j] += delta * u[2]
        p3 = env.descriptor([x, y, z], order=1, normalize=normalize, weights=weights)
        sec_dr2 = p1 + p3 - 2 * p2
    ...
    if failed > 0:
        print("maximum failure (with atol={}): {}".format(atol, max(failures)))
