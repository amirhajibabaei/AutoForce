...
class ClusterSoap:
    ...
    def __init__(self, soap):
        ...
        """
        self.soap = soap
        self.neighbors = NewPrimitiveNeighborList(
            soap.rc, skin=0.0, self_interaction=False, bothways=True, sorted=False
        )
    ...
    def describe(self, pbc, cell, positions, iderive=True, jderive=True):
        ...
        for k in range(n):
            ...
            env = (
                positions[indices]
                + np.einsum("ik,kj->ij", offsets, cell)
                - positions[k]
            )
    ...
    def descriptors_derivatives(
        self, pbc, cell, positions, sumj=False, jsorted=True, selfderive=True
    ):
        """
        Inputs: pbc, cell, positions
        Returns: p, q, i, j
        ...
        * If jsorted is False,
        ...
        if sumj:
            ...
            if jsorted:
                ...
                j = [np.unique(a) for a in np.split(j[k], indices_or_sections=sections)]
                q = np.split(
                    np.concatenate(q, axis=1)[:, k],
...
class TorchSoap(Function):
    ...
    def forward(ctx, pbc, cell, xyz, csoap):
        ...
        p, q, i, j = csoap.descriptors_derivatives(
            pbc, cell, _xyz, sumj=False, jsorted=True
        )
    ...
    def backward(ctx, grad_output):
        ...
        for i, j, dxi_drj in zip(*[ii, jj, q]):
            grads[j] = torch.einsum("ip,pim->m", grad_output[i], dxi_drj)
...
def test_if_works():
    ...
    p, q, i, j = csoap.descriptors_derivatives(
        atoms.pbc, atoms.cell, atoms.positions, sumj=False, jsorted=True
    )
    ...
    p_a, q_a, is_a, js_a = csoap.descriptors_derivatives(
        atoms_a.pbc, atoms_a.cell, atoms_a.positions, sumj=True
    )
    positions_b = flake - center * 0.33
    atoms_b = Atoms(positions=positions_b, cell=cell, pbc=pbc)
    p_b, q_b, is_b, js_b = csoap.descriptors_derivatives(
        atoms_b.pbc, atoms_b.cell, atoms_b.positions, sumj=True
    )
    print(np.allclose(p_a - p_b, 0.0), np.allclose(q_a - q_b, 0.0), (q_a - q_b).max())

    p_ = csoap.descriptors(atoms_b.pbc, atoms_b.cell, atoms_b.positions)
    print(np.allclose(p_a - p_, 0.0))

    # test system with one isolated atom
    atoms_b[-1].position = [0, 0, 0]
    p, q, i, j = csoap.descriptors_derivatives(
        atoms_b.pbc, atoms_b.cell, atoms_b.positions, sumj=False, jsorted=True
    )
    # for a, b in zip(q, i):
...
def test_grad():
    ...
    # dp
    delta = 1e-5
    p1, q1, i1, j1 = csoap.descriptors_derivatives(
        atoms.pbc, atoms.cell, atoms.positions, jsorted=False, sumj=False
    )
    dr = np.random.uniform(-delta, delta, size=atoms.positions.shape)
    atoms.positions += dr
    p2, q2, i2, r2 = csoap.descriptors_derivatives(
        atoms.pbc, atoms.cell, atoms.positions, jsorted=False, sumj=False
    )

    dpr = np.concatenate([a.sum(axis=1, keepdims=True) for a in q1], axis=1) * dr
    dp = dpr.sum(axis=(1, 2))
