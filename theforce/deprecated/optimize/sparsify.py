...
class SparseSampler:
    def __init__(self, cutoff, numbers, atomic_unit=None, lmax=2, nmax=2, exponent=4):
        self.cutoff = cutoff
        self.kern = EnergyForceKernel(
            [
                SoapKernel(
                    Positive(1.0) * Normed(DotProd() ** exponent),
                    atomic_number,
                    numbers,
                    lmax,
                    nmax,
    ...
    def sample_from_atoms(self, atoms, tol=0.01):
        ...
        for loc in atoms:
            if (self.kern(loc, self.sparse) > 1.0 - tol).any():
                continue
            else:
                self.sparse += loc
    ...
    def sample_from_file(self, traj, tol=0.01):
        data = AtomsData(traj=traj, cutoff=self.cutoff, descriptor=self.kern.kernels)
