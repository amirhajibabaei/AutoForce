...
def init_velocities(atoms, temperature, overwrite=False, cm0=True, rot0=True):
    ...
    no_vel = vel is None or np.allclose(vel, 0.0)
...
def write_average_positions(traj, w, sl="::", out=None):
    ...
    cell = moving_average(np.stack([atoms.cell for atoms in read(traj, sl)]), w)
    if out is None:
...
def get_repeat_reciprocal(atoms, spacing=0.1):
    return np.ceil(
        np.linalg.norm(atoms.get_reciprocal_cell(), axis=1) / spacing
    ).astype(int)


def get_repeat(atoms, spacing=10.0):
