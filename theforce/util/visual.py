...
from theforce.util.util import iterable, timestamp
...
def show_trajectory(
    traj, radiusScale=0.3, remove_ball_and_stick=False, preprocess=no_preprocess, sl=":"
):
    if type(traj) == str:
        data = read(traj, sl)
    else:
...
def visualize_leapfrog(
    file, plot=True, extremum=False, stop=None, mlcolor=None, colors=None, tlim=None
):
    energies = []
    temperatures = []
    exact_energies = []
    ...
    if plot:
        ...
        if len(exact_energies) > 0:
            axes[0].scatter(
                *zip(*exact_energies), c=list(map({0: "r", 1: "g"}.get, acc)), zorder=2
            )
            if mlcolor:
