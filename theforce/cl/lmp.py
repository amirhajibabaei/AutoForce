...
def get_cell():
    ...
    cell = np.array([[xhi, xy, xz], [0.0, yhi, yz], [0.0, 0.0, zhi]])
    return cell, pbc
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Dynamics with LAMMPS")
    parser.add_argument(
        "-i", "--input", default="in.lammps", type=str, help="LAMMPS input script"
    )
    args = parser.parse_args()

    # setup lammps
    units, map_numbers, fixID, fixIndex, commands = read_lammps_file(args.input)
    lmp = lammps()
