...
class AtomsGenerator:
    ...
    def __init__(self, atoms, sim=1.-1e-6, forbidden={}, prefix='search', rank=0):
        """
    ...
    def swaps(self, parents, switch_type):
        a = self.generate(self.generate(parents, switch_type),
                          switch_type[::-1])
    ...
    def search_swaps(self, parents, swap_types, epochs=1, max_child=10, max_parents=10):
        for _ in range(epochs):
            ...
            self.log('')
            self.log(
                f'{len(parents)} lowest energies of {len(generation)}')
